...
addCommandHandler("pmacall",
    function(source, commandName, callRecieverName)

        -- no player to call --
        if not callRecieverName then
            outputChatBox(PREFIX.."#ff0000 Syntax:#878787 /pma#ff6666call [player name]", source, 255,100,100, true)
            return
        end

        -- find the player --
        local callRecieverPlayer = nil

        for k, player in pairs(getElementsByType("player")) do

            -- player name without color codes --
            local player_unhexed = removeHex(getPlayerName(player))

            -- if player exists --
            if callRecieverName == player_unhexed then
                callRecieverPlayer = player
                break
            end
        ...
        -- player exists --
        if callRecieverPlayer and isElement(callRecieverPlayer) then

            -- can't call yourself --
            if callRecieverPlayer == source then
                outputChatBox(PREFIX.."#ffffff Imagine calling yourself...", source, 255,100,100, true)
                return
            end

            -- prevent spam calling --
            if getElementData(callRecieverPlayer, "phoneIsRinging") ~= 1 then
                triggerClientEvent(callRecieverPlayer, "playSoundClient", root, source)
            else
                outputChatBox(PREFIX.."#ffffff Sorry,#878787 "..callRecieverName.."'s#ffffff phone is already ringing.", source, 255,100,100, true)
            end

        else
            outputChatBox(PREFIX.."#ff0000 Unknown player", source, 255,100,100, true)
        end

    end
)

function playMinimizeSound (source)
	local players = getElementsByType ("player")
	for i,v in ipairs (players) do
		if v ~= source then
		    triggerClientEvent("onMinimizeTrigger", v, getPlayerName(source))
		end
	end
end

function playRestoreSound (source)
	local players = getElementsByType ("player")
	for i,v in ipairs (players) do
		if v ~= source then
		    triggerClientEvent("onRestoreTrigger", v, getPlayerName(source))
		end
	end
end

addEvent("onMinimizePlaySound", true)
addEvent("onRestorePlaySound", true)

addEventHandler("onMinimizePlaySound", resourceRoot, playMinimizeSound)
addEventHandler("onRestorePlaySound", resourceRoot, playRestoreSound)
