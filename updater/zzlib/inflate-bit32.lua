...
function inflate.bitstream_init(file)
	...
	function bs:peekb(n)
		...
		return bitAnd(self.b,bitLShift(1,n)-1)
	end
...
function hufftable_create(depths)
	...
	for i=1,nvalues do
		...
		if len > 0 then
			...
			local rcode = 0
			for j=1,len do
				rcode = rcode + bitLShift(bitAnd(1,bitRShift(code,j-1)),len-j)
			end
...
function block_loop(out,bs,nlit,ndist,littable,disttable)
	local lit
	repeat
		...
		elseif lit > 256 then
			...
			if lit < 265 then
				size = size + lit - 257
			elseif lit < 285 then
				nbits = bitRShift(lit-261,2)
				size = size + bitLShift(bitAnd(lit-261,3)+4,nbits)
			else
				size = 258
			end
			if nbits > 0 then
			...
			if v < 4 then
				...
				nbits = bitRShift(v-2,1)
				dist = dist + bitLShift(bitAnd(v,1)+2,nbits)
...
function inflate.crc32(s,crc)
	if not crc32_table then
		...
		for i=0,255 do
			local r=i
			for j=1,8 do
				r = bitXor(bitRShift(r,1),bitAnd(0xedb88320,bitNot(bitAnd(r,1)-1)))
			end
			crc32_table[i] = r
		end
	end
	...
	for i=1,#s do
		local c = s:byte(i)
		crc = bitXor(crc32_table[bitXor(c,bitAnd(crc,0xff))],bitRShift(crc,8))
	end
	crc = bitNot(crc)
	if crc<0 then
